...
class Environment:
    ...
    def __init__(self):
        ...
        # Marco la posicion inicial y la final
        self.grilla[self.init_posX][self.init_posY] = 2
        self.grilla[self.finish_posX][self.finish_posY] = 2
    ...
    def accept_action(self,action,x,y):
        if action == "up": #Si se quiere ir hacia arriba verificamos que no se salga de la grilla y que no haya un obstaculo
            ...
            if x > 0 and self.grilla[x-1][y]!= 1 :
                return True    
            return False
        elif action == "down": #Si se quiere ir hacia abajo verificamos que no se salga de la grilla y que no haya un obstaculo
            #print(str(x)+'<'+str(self.sizeX-1)+'')
            if x < self.sizeX - 1 and self.grilla[x+1][y]!= 1:
                return True    
            return False
        elif action == "right": #Si se quiere ir a la derecha verificamos que no se salga de la grilla y que no haya un obstaculo
            #print(str(y)+'<'+str(self.sizeY - 1)+'')
            if y < self.sizeY - 1 and self.grilla[x][y+1]!= 1:
                return True  
            return False
        elif action == "left": #Si se quiere ir a la izquierda verificamos no se salga de la grilla y que no haya un obstaculo
            #print(str(y)+'> 0 ')
